...
app.post('/api/directions', async (req, res) => {
  ...
  try {
    const routePromises = modes.map(mode => {
      const url = `${GOOGLE_DIRECTIONS_API_URL}?origin=${origin}&destination=${destination}&mode=${mode}&alternatives=true&key=${config.GOOGLE_MAPS_API_KEY}`;
    ...
    const routes = responses.flatMap((response, index) => {
      ...
      return response.data.routes.map((route, routeIndex) => {
        if (!route.legs || route.legs.length === 0) {
          console.error(`No legs found for mode ${modes[index]}, route ${routeIndex}`);
          return null;
        }
...
app.get('/api/transport-options', async (req, res) => {
  ...
  const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${destination}&mode=transit&departure_time=1725008400&&alternatives=true&key=${API_KEY}`;
