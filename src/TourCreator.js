...
const TourDisplay = ({ tour, hotel, summary, tips, onSummaryChange, onTipsChange, onSaveTour, isSaving, directions, transportData }) => {
  ...
  const chooseBestRoute = (routes) => {
    ...
    const walkingRoute = routes.find(r => r.mode.toLowerCase() === 'walking');
    const transitRoute = routes.find(r => r.mode.toLowerCase() === 'transit');
    const drivingRoute = routes.find(r => r.mode.toLowerCase() === 'driving');
  ...
  const renderRouteStep = (step) => {
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
        {renderTransportIcon(step.travel_mode)}
  ...
  const renderRouteCard = (routeOptions, index) => {
    if (!routeOptions || !Array.isArray(routeOptions) || routeOptions.length === 0) {
      console.error('Invalid route options:', routeOptions);
      return null;
    }
